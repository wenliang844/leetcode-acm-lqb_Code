...
public class day024_1971寻找图中是否存在路径 {
    public static void main(String[] args) {
        ...
        //7-4-0-8-5
        System.out.println(validPath2(10, new int[][]{{0, 7}, {0, 8}, {6, 1}, {2, 0}, {0, 4}, {5, 8}, {4, 7}, {1, 3}, {3, 5}, {6, 5}}, 7, 5));
    }
    ...
    public static boolean validPath2(int n, int[][] edges, int source, int destination) {
        ...
        int[] binchaji = new int[n];
        for (int i = 0; i < binchaji.length; i++) {
            binchaji[i] = i;
        }
        for (int i = 0; i < edges.length; i++) {
            binchaji[edges[i][1]] = find(edges[i][0], binchaji);
        }

        return false;
    }

    private static int find(int i, int[] binchaji) {
        if (binchaji[i] == i){
            return i;
        }
        return find(binchaji[i],binchaji);
    }
}
