...
所以你应该输出2.
 */
public class day25_455分发饼干 {

    public static void main(String[] args) {
        System.out.println(findContentChildren(new int[]{1, 2, 3}, new int[]{1, 1}));//1
        System.out.println(findContentChildren(new int[]{1, 2, 3,4,5,6}, new int[]{1, 2,3,5,7}));//5
    ...
    public static int findContentChildren(int[] g, int[] s) {
        ...
        思路
        两者排序后
        1.如果s里面的饼干满足g  计数器加1 s的下标加1 g的下标加1
        2.如果s里面的饼干不能满足g   s的下标+1  继续匹配,直到找到一块饼干符合
        退出条件:当s的下标走到了最后
         */
        Arrays.sort(g);
        Arrays.sort(s);
